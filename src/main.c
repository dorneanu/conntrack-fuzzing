...
int main(int argc, char *argv[])
{
	...
	for (i=1; i<argc; i++) {
		...
		case 'X':
			if (++i < argc) {
				strncpy(fuzz_file, argv[i], PATH_MAX);
				if (strlen(argv[i]) >= PATH_MAX){
					fuzz_file[PATH_MAX-1]='\0';
					fprintf(stderr, "Path to fuzz file to long. Cutting it down to %d characters", PATH_MAX);
				}
				break;
			}
			show_usage(argv[0]);
			fprintf(stderr, "Missing fuzz filename\n");
			break;
#endif
		case 'C':
			if (++i < argc) {
